...
<body>
			...
			<ol class="breadcrumb text-capitalize bg-white">
				<li class="breadcrumb-item"><a style="color: #000"
					href='<c:url value='/trang-chu' />' class="text-purple font-15pt">trang
						chủ</a></li>
				<li style="color: #ff5800" class="breadcrumb-item active font-15pt">giỏ
					hàng</li>
			</ol>
			</nav>
			<div class="row">
					...
					<c:if test="${not empty listCart }">
											...
											<td style="text-align: center">
												<div style="display: inline-flex;" class="cart-plus-minus">
													<a
														...
														style="cursor: pointer; margin-right: 3px; background: #ff5800;">-</a>
													<input id="quantityCart_${cart.value.product.productID}"
														...
														class="dec qtybutton btn"
														style="cursor: pointer; margin-left: 3px; background: #ff5800;">+</a>
												</div>
											</td>
											<c:set var="price"
												value="${(cart.value.product.price* (100 - cart.value.product.salePrice))/100}" />
											<td id="price_${cart.value.product.productID}"
												style="text-align: center" class="nowrap-text">${price}
												VND</td>
											<td id="sumPrice_${cart.value.product.productID}"
												style="text-align: center" class="nowrap-text">${price * cart.value.quantity}
												VND</td>
								...
								<tfoot>
									<tr>
										<td colspan="4" class="text-right text-danger font-title">Tổng
											tiền</td>
										<td id="totalPrice" style="text-align: center"
											class="text-danger font-title">${totalPrice}VND</td>
						...
						<div id="mua" class="text-right btn-action-cart"
							...
							<security:authorize access="isAuthenticated()">
								<a href="" style="background: #ff5800;" id="orderCart"
	...
	function editInfoCart(id,quantity, quantityOld) {
	    var price = parseInt($('#price_'+id).text(), 10);// lay gia
	    $('#sumPrice_'+id).text(price*quantity +" vnd");// thay doi gia
	    var totalPrice = parseInt($('#totalPrice').text(), 10) - (price*quantityOld) + (price*quantity) ;
	    $('#totalPrice').text(totalPrice +" vnd");// thay tong gia
