...
public class CategoryAPI {
	...
	@GetMapping("/api/web/category/{id}/{page}/{limit}")
	public ResponseEntity<?> productByGroupPaging(@PathVariable(value = "id", required = false) long id,
			@PathVariable(value = "page", required = false) int page,
			@PathVariable(value = "limit", required = false) int limit) {
		Pageable pageable = new PageRequest(page - 1, limit);
		List<ProductDTO> listProduct = productService.findAllByCatetogyID(id, pageable);
