...
public interface IOrderDetailRepository extends JpaRepository<OrderDetailEntity, Long> {
	...
	@Query("SELECT New com.khosach.dto.RevenueDTO( SUM(od.price),YEAR(o.deliveryDate)) FROM OrderDetailEntity od JOIN od.orders o  Where o.status = 4 and od.status=1 GROUP BY YEAR(o.deliveryDate) ORDER BY YEAR(o.deliveryDate)")
