...
public class OrderServiceImpl implements ICRUDService<OrderDTO>, IOrderService {
	...
	@Override
	@Transactional
	public void save_order(OrderCartDTO orderCartDTO) {
		List<CartDTO> listCartDTO = cartService.findAllByProductIDAndUserID(orderCartDTO.getProductIDs());
		OrderDTO orderDTO = new OrderDTO();
		orderDTO.setPaymentMethods(orderCartDTO.getPaymentMethods());
		orderDTO.setStatus(1);
		orderDTO.setUserid(SecurityUtils.getPrincipal().getUserID());
		save(orderDTO);
		long orderID = findOrderIDMax();
		for (CartDTO cart : listCartDTO) {
			orderDetailService.saveOrderDetail(cart, orderID);
			cartService.delete(cart.getProduct().getProductID());
		}
	}
