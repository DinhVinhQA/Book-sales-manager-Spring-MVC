			...
			success : function(result) {
	            ...
	            if (listSize > 0) {
	            	for (i = 0; i < listSize; i++) {
	            		...
	            		infoRow +='Đơn hàng<a href="/khoSach/chi-tiet-don-hang?id='+result[i].orderID+'">#'+result[i].orderID+'</a></div>'+'<p>Ngày đặt :'+result[i].dateOfIssueString+'</p></div><div class="info-content-top-right"><a href="/khoSach/chi-tiet-don-hang?id='+result[i].orderID+'">QUẢN LÝ</a></div></div><hr>';
	            		var orderDetailSize = result[i].listOrDerDetail.length;		
	            		for (j = 0; j < orderDetailSize; j++) {	
	            			infoRow += '<div class="info-content-bottom"><div class="info-content-bottom-img"><a href=""><img src="/khoSach/image/Product/'+result[i].listOrDerDetail[j].thumbnail+'"></a></div>';
