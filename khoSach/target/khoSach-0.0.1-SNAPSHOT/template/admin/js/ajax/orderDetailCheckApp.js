function clickChiTiet(maHoaDon) {
			$.ajax({
				type : "GET",
						url : '/khoSach/api/orderdetail/' + maHoaDon, //Tên api
						dataType : "json",
						success : function(result) {
							...
							if (listSize > 0) {
								...
								for (i = 0; i < listSize; i++) {
									...
									cell4.innerHTML = "<img style='width: 60px' src='/khoSach/image/Product/"+result[i].thumbnail+"'>"
									cell5.innerHTML = result[i].quantity;
									cell6.innerHTML = result[i].price;
									cell7.innerHTML = result[i].totally;
		...
		function deleteRowOrderDetailAjax(r,orderDetailID) {
			$.ajax({
				url : '/khoSach/api/orderdetail/'+orderDetailID,
				type : 'DELETE',
				success : function(result) {
					swal("Xóa thành công", {
					      icon: "success",
